...
class InkyFrame57 : public PollingComponent, public display::DisplayBuffer,
 ...
 protected:
  ...
  // Pimoroni Inky Frame 5.7" Hardcoded Pinout
  const int CS_PIN = 17;
  const int DC_PIN = 28;
  ...
  void wait_busy() {
    while (is_busy()) {
      delay(50);
      yield(); // Prevent watchdog timeouts (7-color displays take ~30s to update)
    }
  }

 public:
  // Update every 60 seconds (frequent updates can damage 7-color e-ink)
